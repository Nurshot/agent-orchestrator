...
export function ExecutionContextView({
  ...
  if (repositories.length > 0)
    facts.push({ label: labels.repository, value: repositories.join(", ") });
  ...
  if (workerAgent && workerAgent !== activeAgent) {
    facts.push({
      label: `${labels.worker} (${labels.configured})`,
      value: workerAgent,
    });
  }
  if (orchestratorAgent && orchestratorAgent !== activeAgent) {
    facts.push({
      label: `${labels.orchestrator} (${labels.configured})`,
        ...
        >
          ...
          {displayBranch ? (
            <>
              ...
              <span className="truncate" title={displayBranch}>
                {displayBranch}
              </span>
            </>
          ) : null}
    ...
    >
      ...
      {loading ? (
        <p
          aria-busy="true"
          aria-label={labels.loading}
          className="mt-2 text-xs text-passive"
          role="status"
        >
          {labels.loading}
        </p>
      ...
      {!loading ? (
        <>
          ...
          {path ? (
            <div
              className="mt-2 flex min-w-0 items-center gap-1 text-2xs text-passive"
              title={path}
            >
              <span
                aria-hidden="true"
                className="size-1.5 shrink-0 rounded-full bg-passive"
              />
