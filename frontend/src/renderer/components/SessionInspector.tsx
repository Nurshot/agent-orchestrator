...
import {
  ...
  useSessionWorkspaceFilesChangedCount,
...
type OpenReviewerTerminal = (target: {
  handleId: string;
  harness: string;
}) => void;
...
export const SessionInspector = memo(function SessionInspector({
  ...
  const [browserTopbarHost, setBrowserTopbarHost] =
  ...
  const localWorkspaceData = session
    ? inspectorQueryClient.getQueryData<{ files?: unknown[] }>(
        sessionWorkspaceFilesQueryKey(session.id),
      )
    : undefined;
  const {
  ...
  const cloudReview = useQuery({
    ...
    enabled:
      cloudReady && session?.cloud !== undefined && cloudOrgId !== undefined,
  ...
  const filesChangedCount = session?.cloud
    ? cloudFilesChangedCount
      ? cloudFilesChangedCount
      : undefined
    : localFilesChangedCount;
  const hasWorkspaceInventory = session?.cloud
    ? Boolean(cloudReview.data?.files.length)
    : Boolean(localWorkspaceData?.files?.length);
  ...
  const tabs = availableViewDefs.map((entry) => {
    ...
    return {
      ...
      displayLabel:
        ...
        (filesChangedCount > 0 || hasWorkspaceInventory)
      ...
      <SessionInspectorShellView
        ...
        headerActions={
          view === "browser" && !browserPoppedOut ? (
            <>
              <div
                className="browser-panel__topbar-host min-w-0 flex-1"
                ref={setBrowserTopbarHost}
              />
              <span
                aria-hidden="true"
                className="session-inspector-actions-spacer"
              />
            </>
          ) : (
            <span
...
function reviewsTabVisible(session: WorkspaceSession | undefined): boolean {
  ...
  return sortedPRs(session).some(
    (pr) => pr.state === "open" || pr.state === "draft",
  );
}

function externalReviewActorMatchesPRAuthor(
...
const SummaryView = memo(function SummaryView({
  ...
  const showWorkers =
    ...
    (session.cloud !== undefined || usePreviewData);
  return (
    <SessionInspectorSummaryView
      activity={
        <>
      ...
      workers={
        showWorkers ? (
          <OrchestratorChildrenSection session={session} />
        ) : undefined
      }
      usage={
        showUsageError ? (
...
function InspectorPolicyRow({
  ...
  return (
    <div
      ...
      data-slot="inspector-policy-row"
    >
      <div className="flex min-w-0 items-center gap-1.5">
        ...
        {description ? (
          <Tooltip>
            ...
            <TooltipContent className={cn("leading-normal", tooltipClassName)}>
              {description}
            </TooltipContent>
          </Tooltip>
        ) : null}
...
function UsageCostTelemetry({ usage }: { usage: SessionUsage }) {
  ...
  const showsAgentCost = usage.harnesses.some(
    (harness) => harness.totals.estimatedCost !== null,
  );
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="min-w-0">
          <p className="text-2xs text-settings-muted">
            {t("inspector.usage.processedTokens")}
          </p>
          <p
            aria-label={
          ...
          >
            {processedTokens === null
              ? t("inspector.usage.noUsageYet")
              : formatTelemetryTokenValue(processedTokens)}
          </p>
        </div>
        <div className="min-w-0 text-right">
          <div className="flex items-center justify-end gap-1">
            <p className="text-2xs text-settings-muted">
              {t("inspector.usage.estimatedCost")}
            </p>
...
function UsageAgentAttribution({
  harness,
}: {
  ...
  const modelSummaryTitle =
    harness.models.length === 1 ? harness.models[0].modelId : modelSummary;
  const attribution = (
    <>
      <AgentAvatar className="size-4" decorative provider={harness.harness} />
      <span className="shrink-0 text-sm-md text-settings-label">
        {harnessName}
      </span>
      {modelSummary ? (
        <>
          ...
          <span
            className="truncate text-2xs text-settings-muted"
            title={modelSummaryTitle ?? undefined}
          >
            {modelSummary}
          </span>
        </>
        ...
        <>
          ...
          >
            {open ? (
              <ChevronDown
                aria-hidden="true"
                className="size-3 shrink-0 text-settings-muted"
              />
            ) : (
              <ChevronRight
                aria-hidden="true"
    ...
    mutationFn: async (autoInjectCI: boolean) => {
      ...
      if (error)
        throw new Error(
          apiErrorMessage(
            error,
            t("inspector.ci.autoInjectError", { status: response.status }),
          ),
        );
    },
...
function ProviderUsageDetails({
  ...
  const { t } = useTranslation();
  const showCost = harness.models.some(
    (model) => model.totals.estimatedCost !== null,
  );
  ...
  return (
    <div>
      {harness.models.length > 0 ? (
        ...
        <p className="px-1 py-1 text-2xs text-settings-muted">
          {t("inspector.usage.noModelTelemetry")}
        </p>
      )}
    </div>
  );
...
function AutoInjectReviewPolicyControl({
  session,
}: {
  session: WorkspaceSession;
}) {
  ...
  const save = useMutation({
    mutationFn: async (autoInjectReview: boolean) => {
      ...
      if (error)
        throw new Error(
          apiErrorMessage(error, t("inspector.review.autoInjectError")),
        );
    },
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: workspaceQueryKey });
...
function UsageDisclosureRow({
  ...
  return (
    <div className="px-1 py-0.5">
      ...
      >
        <span
          className={`flex min-w-0 items-center gap-1 text-settings-label ${nameClassName}`}
        >
          {open ? (
            <ChevronDown
              aria-hidden="true"
...
function formatCacheHitRate(
  ...
  const percentage = Math.min(
    100,
    Math.max(0, (cachedInputTokens / inputTokens) * 100),
  );
...
function hasMeaningfulSessionUsage(
  usage?: SessionUsage,
...
function formatModelName(modelID: string): string {
  ...
  if (isClaude) {
    ...
    const familyIndex = parts.findIndex((part) =>
      ["haiku", "sonnet", "opus"].includes(part.toLowerCase()),
    );
    if (familyIndex >= 0) {
      const family = parts[familyIndex];
      parts = [
        family,
        ...parts.slice(0, familyIndex),
        ...parts.slice(familyIndex + 1),
      ];
    }
  }
...
function SessionControls({ session }: { session: WorkspaceSession }) {
    ...
    mutationFn: async (terminateOnPrMerge: boolean) => {
      ...
      if (error)
        throw new Error(
          apiErrorMessage(
            error,
            `Failed to update merge policy (${response.status})`,
          ),
        );
    },
  ...
  const policyError =
    policy.error instanceof Error ? policy.error.message : null;
  ...
  const confirmTermination = () => {
    ...
    const nextNav = resolveNextNavigationAfterSessionKill(
      workspace,
      session.id,
    );
    ...
    <Section title={t("inspector.sessionControls")}>
      ...
      {session.kind === "orchestrator" ? null : canTerminateNow ? (
        ...
        <>
          ...
          {policyError ? (
            <p
              className="mt-1 text-2xs leading-normal text-error"
              role="status"
            >
              {policyError}
            </p>
...
function PRSummaryCard({
  ...
  const mergePr = useMutation({
    mutationFn: async () => {
      ...
      if (error)
        throw new Error(
          apiErrorMessage(error, t("pr.merge.failed", { number: pr.number })),
        );
    },
    onSuccess: async () => {
      await Promise.all([
  ...
  const mergeError =
    mergePr.error instanceof Error ? mergePr.error.message : null;
  ...
  return (
    <InspectorPullRequestCardView
      countNounLabel={(count, noun) =>
        `${count} ${t(prNounKeys[noun], { count })}`
      }
      externalIcon={
        <ArrowUpRight
          aria-hidden="true"
...
function ActivityTimeline({
    ...
    pushEvent(
      {
        ...
        content: (
          <PRTimelineLink
            pr={pr}
            verb={appI18n.t("inspector.timeline.draft")}
          />
        ),
    ...
    pushEvent(
      {
        ...
        content: (
          <PRTimelineLink
            pr={pr}
            verb={appI18n.t("inspector.timeline.opened")}
          />
        ),
    ...
    pushEvent(
      {
        ...
        content: (
          <PRTimelineLink
            pr={pr}
            verb={appI18n.t("inspector.timeline.merged")}
          />
        ),
        timestamp: prStateTime(pr),
      },
  ...
  const activityAt =
    session.activity?.lastActivityAt ?? session.updatedAt ?? session.createdAt;
  pushEvent(
    {
      tone: "now",
      content: (
  ...
  return (
    <InspectorActivityTimelineView
      events={[...events].sort((a, b) => b.sortTime - a.sortTime)}
    />
  );
}
...
function InspectorActivityPill({
  activity,
}: {
  activity?: WorkspaceSession["activity"];
}) {
...
function TimelinePill({
  ...
  return (
    <span
      className="inline-flex shrink-0 whitespace-nowrap text-xs font-semibold"
      style={{ color: tone }}
    >
      {label}
    </span>
...
function ReviewsSection({
  ...
  const triggerReview = useMutation({
    mutationFn: async () => {
      ...
      if (error)
        throw new Error(
          apiErrorMessage(error, t("inspector.unableStartReview")),
        );
    ...
    onSuccess: ({ data, reused }) => {
      ...
      const started = data?.reviews?.find(
        (review) => review.status === "running" && review.latestRun,
      );
  ...
  const cancelReview = useMutation({
    mutationFn: async () => {
      ...
      if (error)
        throw new Error(
          apiErrorMessage(error, t("inspector.unableCancelReview")),
        );
    },
    onSuccess: () => {
      setReviewNotice(null);
  ...
  const killReview = useMutation({
    mutationFn: async () => {
      ...
      if (error)
        throw new Error(
          apiErrorMessage(error, t("inspector.unableKillReviewSession")),
        );
      return data;
    },
    onSuccess: (data) => {
  ...
  const githubReviews = prSummaries.filter(
    (pr) =>
      ...
      ((pr.review?.reviews?.length ?? 0) > 0 ||
        (pr.review?.unresolvedBy ?? []).some(
          (reviewer) => reviewer.count > 0,
        ) ||
...
function MergedReviewsSection({
  ...
  const byNumber = new Map<
    number,
    { ao?: PRReviewState; github?: SessionPRSummary }
  >();
  ...
  const requestRereview = async (review: InspectorGithubReview) => {
    ...
    if (error)
      throw new Error(apiErrorMessage(error, "Unable to request re-review"));
  };
  const resolveInlineComment = async (comment: InspectorInlineComment) => {
    ...
    if (error)
      throw new Error(
        apiErrorMessage(error, "Unable to resolve review comment"),
      );
    void queryClient.invalidateQueries({
  ...
  const sendInlineCommentToWorker = async (
    ...
    if (error)
      throw new Error(
        apiErrorMessage(error, "Unable to send review comment to worker agent"),
      );
  };
  const sendReviewSummaryToWorker = async (
    summary: InspectorReviewSummaryAction,
  ) => {
    const { error } = await apiClient.POST(
      "/api/v1/sessions/{sessionId}/send",
      {
    ...
    if (error)
      throw new Error(
        apiErrorMessage(error, "Unable to send review summary to worker agent"),
      );
  };
  const groups: InspectorReviewGroup[] = rows
    .map(([number, { ao, github }]) => {
      const aoRuns = ao
        ? [...(runsByPR.get(ao.prUrl) ?? [])].sort((a, b) =>
            b.createdAt.localeCompare(a.createdAt),
          )
        : [];
      const entries = (github?.review?.reviews ?? []).filter(
        (entry) =>
          !externalReviewActorMatchesPRAuthor(entry.reviewerId, github?.author),
      );
      const allUnresolvedReviewers = github?.review?.unresolvedBy ?? [];
      const allResolvedReviewers = github?.review?.resolvedBy ?? [];
      const agentReviewIds = new Set(
        aoRuns.map((run) => run.githubReviewId).filter(Boolean),
      );
      ...
      const unresolvedReviewers = partitionReviewerComments(
        allUnresolvedReviewers,
        false,
      ).filter(
        (reviewer) =>
          !externalReviewActorMatchesPRAuthor(
            reviewer.reviewerId,
            github?.author,
          ),
      );
      const resolvedReviewers = partitionReviewerComments(
        allResolvedReviewers,
        true,
      ).filter(
        (reviewer) =>
          !externalReviewActorMatchesPRAuthor(
      ...
      const unresolved = unresolvedReviewers.reduce(
      ...
      for (const reviewer of unresolvedByReviewer.values()) {
        ...
        externalEntries.push({
          body: undefined,
          canRequestRereview: canRequestPRRereview(
            "changes_requested",
            github?.url,
          ),
      ...
      return {
        ao: ao
          ? {
              ...
              historical:
                ...
                (!ao.latestRun ||
                  ao.latestRun.status === "failed" ||
                  ao.latestRun.status === "cancelled"),
          ...
          ? {
              ...
              notInjected:
                ...
                unresolvedReviewers.some((reviewer) =>
                  reviewer.links.some(
                    (link) => link.autoInjectReview === false,
                  ),
                ),
              unresolved,
        ...
        meta: [
          ...
          unresolved > 0
            ? t("inspector.unresolvedCount", { count: unresolved })
            : null,
        ]
          .filter(Boolean)
          .join(" · "),
    ...
    .filter((group) =>
      Boolean(
        group.ao ||
        (group.github &&
          (group.github.entries.length > 0 || group.github.unresolved > 0)),
      ),
    );
  return (
    <InspectorReviewsView
      ...
      onViewInlineCommentInFile={(comment) => {
        if (comment.file)
          onOpenReviewFile?.({ line: comment.line, path: comment.file });
      }}
...
function formatInlineReviewCommentMessage(
  ...
  const reviewer = sanitizeWorkerMessagePart(
    comment.reviewerId?.trim() || "unknown reviewer",
  );
  const file = sanitizeWorkerMessagePart(comment.file?.trim() || "");
  const location = file
    ? `${file}${comment.line ? `:${comment.line}` : ""}`
    : "general PR comment";
  const body = sanitizeWorkerMessagePart(
    comment.body?.trim() || "No comment body provided.",
  );
...
function formatReviewSummaryMessage(
  summary: InspectorReviewSummaryAction,
): string {
  const reviewer = sanitizeWorkerMessagePart(
    summary.reviewerId.trim() || "reviewer",
  );
  const body = sanitizeWorkerMessagePart(summary.body.trim());
  const url = sanitizeWorkerMessagePart(
    (summary.url || summary.pullRequestUrl || "").trim(),
  );
  const source =
    summary.source === "agent" ? "AO agent review" : "external PR review";
  const lines = [
...
function projectConfig(
  project: components["schemas"]["ProjectOrDegraded"] | undefined,
...
function ReviewPanel({
  ...
  const [dismissedAutoFailureId, setDismissedAutoFailureId] = useState<
    string | null
  >(null);
  useEffect(() => {
    if (!latestAutoFailure || latestAutoFailure.id === dismissedAutoFailureId)
      return;
    const timer = window.setTimeout(
      () => setDismissedAutoFailureId(latestAutoFailure.id),
      10_000,
    );
    return () => window.clearTimeout(timer);
  ...
  const autoReviewFailure =
    latestAutoFailure && latestAutoFailure.id !== dismissedAutoFailureId
      ? latestAutoFailure.body.trim()
      : null;
      ...
      <Section surface title={t("inspector.review.controls")}>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  <Info
                    aria-hidden="true"
                    className="mt-px size-icon-2xs shrink-0"
                  />
                  ...
                  <span className="min-w-0">
                    {t("inspector.reviewAlreadyRanShort")}
                  </span>
                </button>
              </TooltipTrigger>
          ...
          <div className="flex min-h-10 min-w-0 items-center justify-between gap-3 py-2">
            ...
            <div className="flex min-w-0 items-center justify-end gap-1.5">
              <Button
                ...
                disabled={
                  reviewRunning
                    ? isCancelling || isKilling || isSwitchingReviewer
                    : runDisabled || autoReviewEnabled
                }
        ...
        {reviewLive ? (
          <div className="mt-3 flex items-center gap-2 border-t border-border pt-3">
            <Loader2
              aria-hidden="true"
              className="size-icon-sm shrink-0 animate-spin text-muted-foreground"
            />
...
function runsByPRFrom(
  openReviewStates: PRReviewState[],
  runs: ReviewRunFacts[],
): Map<string, ReviewRunFacts[]> {
...
function triggeredReviewStatesFrom(
  openReviewStates: PRReviewState[],
  runs: ReviewRunFacts[],
): PRReviewState[] {
  return openReviewStates.filter(
    (reviewState) =>
      ...
      runs.some(
        (run) => run.prUrl === reviewState.prUrl && reviewRunHasOutcome(run),
      ) ||
  ...
  if (browserPoppedOut) {
    return (
      ...
      >
        ...
        >
          <p className="text-md-sm text-muted-foreground">
            {t("inspector.browserInCenter")}
          </p>
          <Button
  ...
  return (
    <div role="tabpanel">
      <div
        className={cn(
          inspectorEmptyClass,
          "flex flex-col items-center gap-2 px-5 py-10 text-center",
        )}
      >
        <p className="text-md-sm text-muted-foreground">
          {t("inspector.filesUnavailable")}
        </p>
        <Button
