...
describe("cloud control-plane session lifecycle", () => {
	it("prepares and commits one hidden session with stable mutation keys", async () => {
		...
		expect(new Headers(fetchMock.mock.calls[1]?.[1]?.headers).get("Idempotency-Key")).toBe("commit-key");
	});
