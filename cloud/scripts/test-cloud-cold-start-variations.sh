...
if [[ "${1:-}" == "--measure-startup" ]]; then
    printf '%s\n' '{"agentLaunchStartedMs":80,"agentReadyMs":90,"browserReadyMs":12,"checkoutCompletedMs":50,"checkoutStartedMs":40,"earlyMessageAcceptedMs":3,"earlyMessageDeliveredMs":95,"restoreCompletedMs":70,"restoreStartedMs":50,"runtimeRunningMs":30,"sandboxProvisioningMs":10,"sessionAcceptedMs":2,"workerConnectedMs":30,"workerReadyMs":35,"workspaceReadyMs":70}' > "$AO_CLOUD_STARTUP_RESULT_FILE"
fi
EOF
