...
func TestClaudeModelsRejectProviderCacheWhenDiscoveryFingerprintChanges(t *testing.T) {
	...
	if len(got.Models) != 2 || got.Models[0].ID != "sonnet" || got.Models[1].ID != "opus" ||
		got.Source != "catalog" || !got.Stale {
		t.Fatalf("catalog = %#v, want current-context fallback after fingerprint change", got)
	}
}

func TestClaudeModelsRejectAgentWideProviderCacheWithDifferentFingerprint(t *testing.T) {
	validatedAt := time.Now().Add(-time.Hour)
	...
	if len(got.Models) != 2 || got.Models[0].ID != "configured-gateway-model" || got.Models[1].ID != "sonnet" ||
		got.Source != "catalog" || !got.Stale {
		t.Fatalf("catalog = %#v, want current-context fallback instead of another project's provider IDs", got)
	}
}
