...
- Chat drivers for the user's installed Codex (native app-server), Claude Code
  (claude-agent-acp), Auggie (native `auggie --acp`), Cursor, OpenCode, Droid,
  Kimchi, Kimi, Pi, and OMP. OMP Chat uses native `omp acp` and requires OMP
  15.0.0 or newer. Pi's independently
